...
Управление:
- Drag вершины — двигать точку
- Двойной клик по линии (близко к ребру) — добавить точку
"""
...
class DraggableVertex(QGraphicsEllipseItem):
    ...
    RADIUS = 10  # увеличенные точки для удобства
...
class PolygonScene(QGraphicsScene):
    ...
    CLICK_THRESHOLD = 30.0  # px — макс. расстояние до ребра для добавления точки
    ...
    # ------------------------------------------------------------------
    # Двойной клик — добавление точки на ребро
    # ------------------------------------------------------------------
    def mouseDoubleClickEvent(self, event):
        pos = event.scenePos()
        idx, dist = self._find_nearest_edge(pos)
    ...
    def _add_vertex(self, pos: QPointF):
        ...
        idx, _ = self._find_nearest_edge(pos)
    ...
    # ------------------------------------------------------------------
    # Геометрия: ближайшее ребро
    # ------------------------------------------------------------------
    def _find_nearest_edge(self, pos: QPointF) -> Tuple[int, float]:
        """Вернуть (индекс вставки, расстояние до ближайшего ребра)."""
